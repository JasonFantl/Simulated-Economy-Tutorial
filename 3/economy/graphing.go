...
func updateGraph() {
	for actor := range actors {
		for good, market := range actor.markets {
			...
			previousDataPoints[good][actor] = append(previousDataPoints[good][actor], dataPoint{market.expectedMarketValue, actor.currentPersonalValue(good), market.expectedMarketValue < actor.currentPersonalValue(good)})
		}
	}
}
